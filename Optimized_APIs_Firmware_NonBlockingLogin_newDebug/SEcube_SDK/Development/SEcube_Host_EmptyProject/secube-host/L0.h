...
/**
 *  \brief Reset USEcube to BOOT mode
 *
 *  \param [in] device pointer to SEcube device structure
 *  \param [in] serialno Serial Number to be set on SEcube device
 *  \return Error code or SE3_OK
 *
 *  \details Reset USEcube to BOOT mode by erasing the boot signature; needed to upload a new firmware to the device which has not a JTAG interface
 */
uint16_t L0_bootmode_reset(se3_device* device, const uint8_t* serialno);
