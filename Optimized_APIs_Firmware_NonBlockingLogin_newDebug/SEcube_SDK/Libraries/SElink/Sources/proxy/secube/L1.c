...
uint32_t L1_config(se3_session* s, uint16_t type, uint16_t op, uint8_t* value) {
	...
	// resp::value e' vuoto (zeri) se op=SET
	// req::value e' vuoto (zeri) se op=GET

	uint16_t datalen = 0, resp_len = 0;
	uint16_t error = 0;
	uint8_t* session_data = s->buf + SE3_RESP1_OFFSET_DATA;

	SE3_SET16(session_data, SE3_CMD1_CONFIG_REQ_OFF_ID, type);   //set type   // per SE3_..._ID intendi se USER o ADMIN (cioè type?)
	...
	error = L1_TXRX(s, SE3_CMD1_CONFIG, 0, datalen, &resp_len);
	if (error != SE3_OK) {
		return error;
	}
...
uint16_t key_list(se3_session* s, uint16_t skip, uint16_t max_keys, se3_key* key_array, uint16_t* count) {
	uint16_t error = 0;
	uint16_t resp_len = 0;
	uint16_t n_keys = 0;
	uint16_t i = 0;
	...
	for (i = 0; i < n_keys; i++) {   // Fills with key received
		...
		offset_key += 4 + 4 + 2 + 2 + key_array[i].name_size;   // id + validity + data_size + name_size + name[name_size]
	}
...
uint16_t L1_key_list(se3_session* s, uint16_t skip, uint16_t max_keys, se3_key* key_array, uint16_t* count) {
	...
	while (i != max_keys &&
		(return_value = key_list(s, i + skip, max_keys - i, &(key_array[i]), &xcount)) == 0 &&
		xcount>0){
		i += xcount;
	}
	*count = i;
...
bool L1_find_key(se3_session* s, uint32_t key_id){
	...
	int i = 0, j = 0;
	while ((ret = L1_key_list(s, i, i + FIND_KEY_NUM, key_array, &count)) == SE3_OK && count>0){
		for (j = 0; j<count; j++){
